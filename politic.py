...
for i in range(1,4000):#目標頁數
    ...
    for u in room: #開始一頁一頁去抓
        ...
        try:
            ...
            #去除掉文末 -- 抓出內文 main_content

            main_content = content[1].replace('--', '').replace('\r', ' ').replace('\n', ' ')
        except:
            break
...
def store(data):
    with open('reptt.json','a',encoding='utf8') as f:
        f.write(data.encode(sys.stdin.encoding, "replace").decode(sys.stdin.encoding))
...
for i in range(1,4000):#目標頁數
    ...
    for m in range(i,t):#每頁迴圈解析 若標題含有關鍵字者加入該超連結URL至ROOM
        ...
        for g in news:
            try:
                blog = g.find("a")["href"]
                if "新聞" in  g.select('.title')[0].text and "R" in g.select('.title')[0].text :
                    room.append(blog)
                    print(g.select('.title')[0].text)
            except:
                break
    ...
    #存成json
    try:
        d = {"作者": author, "標題": title, "日期": date,"推噓文總數:":num}
        json_data = json.dumps(d, ensure_ascii=False, indent=4, sort_keys=True)
        store(json_data)
    except:
        continue
